/* ============================================================================
    CTREE_rwvrec()   - Atualiza Registro em arquivo Variavel de Tamanho.
...
INT2 CTREE_rwvrec( arquivo, extra_buffer, tamanho_buffer_extra)
INT2       arquivo;
char      *extra_buffer;
VRLEN     tamanho_buffer_extra;
{
 ...
 if( VIDEO_ALLOC_MEM( sai_buffer, buf_tam_sai) == VD_NULO)
   {
    VIDEO_mostra_mens_erro_alloc(ALOCACAO_RWVLEN);
    return(-3);
   }
 ...
 if( (ret=DATABASE_ReWriteVRecord(real_arquivo, sai_buffer, buf_tam_sai))!=0)
  {
   VIDEO_FREE_ALLOC_MEM(sai_buffer);
