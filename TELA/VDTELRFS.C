...
INT2 VIDEO_TELA_refresh_imagens(tela,param)
...
VIDEO_NUCLEO_SERV        **param;
{
     ...
     if (numimag > 1)
        {
         ...
         if (VIDEO_ALLOC_MEM(refresh,numimag*sizeof(VIDEO_TELASTRRFS))
             == VD_NULO)
            {
             VIDEO_mostra_mens_erro_alloc(ALOCACAO_REFRESH_TELA);
             return(TRUE);
            }
 ...
 if (!(telaaux->imagens+telaaux->imagem_accept)->seqdisp)
     if (!telaaux->imagem_accept)
         VIDEO_TELA_refresh_tela(tela,param);
       else
